...
The comparison runs the same link workload against SQLite in-memory and file databases and four Doublets layouts: united/split and volatile/non-volatile. It measures create, update, delete, enumerate all, and queries by identity, concrete `(source, target)`, outgoing source, and incoming target. The Rust suite also measures creation, reading, and deletion of object-like blog posts on every storage variant.

Preparation and cleanup are outside the measured region. Pull requests run a reduced-scale validation and preserve raw output, tables, and charts as workflow artifacts. Full runs on `main` publish the tables and charts below with a link to the producing workflow run.
...
> Results will be generated by the full benchmark workflow after this change reaches `main`.
